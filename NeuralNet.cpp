...
double NeuralNet::Activate(std::vector<double> &v) {
...
void NeuralNet::Mutate() {
    std::vector<double>::iterator it = fWeights.begin(), end = fWeights.end();
    for (; it != end; ++it) {
...
bool NeuralNet::Save(const char *fileName) {
...
NeuralNet NeuralNet::Load(const char *fileName) {
    NeuralNet nn;
    ...
    if(!in) {
        ...
        //std::cerr << "loading neural net from file " << fileName << std::endl;
        in.read((char*)&nn.fNumInputs, sizeof(int));
        in.read((char*)&nn.fNumHidden, sizeof(int));
        int totalWeights = (nn.fNumInputs + 1)*nn.fNumHidden + nn.fNumHidden+1;
        for(int i = 0; i < totalWeights; ++i) {
            double x;
...
double NeuralNet::GetRandomWeight() {
    double r = GetRandomNumber();
    double res = 2 * r - 1;
    return res;
}

double NeuralNet::GetRandomNumber() {
    double res = rand();
    res /= RAND_MAX;
    return res;
}

double NeuralNet::Tanh(double x) {
    if(x > +20.0) {
        return +1.0;
    } else if(x < -20.0) {
        return -1.0;
...
bool NeuralNet::AreEqualWithEpsilon(double x, double y) {
...
bool operator == (const NeuralNet &nn1, const NeuralNet &nn2) {
    ...
    for(int i = 0; i < totalWeights; ++i) {
        if(fabs(nn1.fWeights[i] - nn2.fWeights[i]) > 1e-15) {
            std::cout << i+1 << "th weights are different " << nn1.fWeights[i] << " # " << nn2.fWeights[i] << std::endl;
            return false;
        }
    }
...
std::ostream& operator << (std::ostream &out, NeuralNet &nn) {
